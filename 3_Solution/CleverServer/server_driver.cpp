...
protected:
	...
	virtual void OnMessage(std::shared_ptr<clever::connection<clever::MessageType>> client, clever::message<clever::MessageType>& msg)
	{
		...
		case clever::MessageType::PATGetCashRequest:
		{
			...
			try
			{
				OnGetCashPAT(l_pat, l_cashValue, l_currencyCashISO);
			...
			msg << responseBack << l_cashValue << l_currencyCashISO;
			client->Send(msg);
		}
		break;
		...
		case clever::MessageType::UsernameGetCashRequest:
		{
			...
			try
			{
				OnGetCashUsername(l_pat, l_cashValue, l_currencyCashISO);
				msg << l_cashValue << l_currencyCashISO;
				client->Send(msg);
			}
			catch (...)
			{
		...
		{
			std::cout << "[" << client->GetID() << "]: Add cash request\n";
			char l_pat[1024]; msg >> l_pat;
			...
			try
			{
				...
				OnAddCashPAT(l_pat, l_cashValue);
		...
		case clever::MessageType::AddCashUsernameRequest:
		{
			...
			try
			{
				...
				OnAddCashUsername(l_username, l_cashValue);
				strcpy(responseBack, "SuccessAddCash");
			}
			catch (...)
			{
