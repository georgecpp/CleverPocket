...
EditCardDialog::EditCardDialog(const QString& PAT, QWidget* parent)
	:EditCardDialog(parent)
{
	this->currPATLogged = PAT.toStdString();
	this->currUsernameLogged = "";
}

EditCardDialog::EditCardDialog(const std::string& username, QWidget* parent)
	: EditCardDialog(parent)
{
...
void EditCardDialog::on_editCardPushButton_clicked()
{
	...
	if (!c.Incoming().empty() && stillConnectedWaitingAnswer)
	{
		...
		if (msg.header.id == clever::MessageType::ServerEditCardResponse)
		{
...
void EditCardDialog::on_cancelEditCardPushButton_clicked()
{
	this->done(QDialog::Rejected);
}

void EditCardDialog::on_validDateEdited()
{
...
void EditCardDialog::on_cardNumberEdited()
{
...
void EditCardDialog::loadCurrencyISOS()
{
