...
namespace clever
{
	...
	protected:
		...
		void OnGetCashUsername(char username[], std::string& cashValue, std::string& cashCurrencyISO, clever::CredentialHandler& userInfo, std::string& mailState)
		{
			...
			std::string queryGetAllUserInformation = "SELECT [First Name], [Last Name], [Username], [Email], [Country], [PhoneNumber], [Picture] FROM CleverPocket.dbo.Users WHERE  UserID = " + userID;
			...
			std::stringstream ss(resultQueryGetAllUserInfoCards);
				...
				country = convertToSqlVarcharFormat(country.c_str());
				query = "SELECT [CountryName] FROM CleverPocket.dbo.Countries WHERE CountryID = " + country;
				ExecQuery(query);
				userInfo.setCountryID(query);
